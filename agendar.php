...
if (isset($_GET['action']) && $_GET['action'] === 'buscar_horarios') {
    ...
    foreach ($ocupados as $ag) {
        ...
        $inicioMin = ($horaMin[0] * 60) + $horaMin[1];
        $fimMin = $inicioMin + 30; // Bloqueio padrão de 30min
        for ($m = $inicioMin; $m < $fimMin; $m++) $minutosOcupados[$m] = true;
    }
    ...
    foreach ($turnos as $turno) {
        ...
        $inicioMin = ($inicioParts[0] * 60) + $inicioParts[1];
        $fimMin = ($fimParts[0] * 60) + $fimParts[1];

        for ($time = $inicioMin; $time <= ($fimMin - $duracaoServico); $time += 30) {
            $livre = true;
            for ($check = $time; $check < ($time + $duracaoServico); $check++) {
                if (isset($minutosOcupados[$check])) { $livre = false; break; }
            }
            if ($livre) {
                $horaFormatada = str_pad(floor($time / 60), 2, '0', STR_PAD_LEFT) . ':' . str_pad($time % 60, 2, '0', STR_PAD_LEFT);
...
// 2. Buscar Cliente por CPF (NOVO)
if (isset($_GET['action']) && $_GET['action'] === 'buscar_cliente') {
    $cpf = preg_replace('/[^0-9]/', '', $_GET['cpf']); // Limpa pontuação
    ...
    header('Content-Type: application/json');
    if ($cliente) {
        echo json_encode(['found' => true, 'nome' => $cliente['nome'], 'telefone' => $cliente['telefone']]);
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome && $horario && $servicoNome && $cpf) {
        ...
        if ($clienteExistente) {
            // Cliente já existe: Atualiza dados (telefone/nome) caso tenha mudado
        ...
        // Concatena a observação do cliente com info técnica
        $obsFinal = $obsCliente;
        
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$profissionalId, $clienteId, $nome, $cpf, $servicoNome, $data, $horario, $obsFinal]);
...
<div class="container">
    ...
    <div class="success-screen">
        ...
        <p>Seu horário está reservado. Já o pode ver na sua agenda.</p>
        <a href="agendar.php" class="btn-action" style="display:inline-block; text-decoration:none;">Novo Agendamento</a>
    </div>
    <?php else: ?>
...
<script>
    ...
    function buscarCliente() {
        ...
        if (cpf.length < 11) return; // Só busca se tiver tamanho mínimo

        feedback.style.display = 'block';
        feedback.innerText = 'Buscando cadastro...';
        feedback.className = 'feedback-msg'; // reset cor

        fetch(`agendar.php?action=buscar_cliente&cpf=${cpf}`)
            .then(r => r.json())
            .then(data => {
                if(data.found) {
                    feedback.innerText = 'Cliente encontrado! Seus dados foram preenchidos.';
                    feedback.classList.add('text-success');
                    nomeInput.value = data.nome;
                    telInput.value = data.telefone;
                } else {
                    feedback.innerText = 'CPF não cadastrado. Por favor, preencha seus dados.';
