            ...
            <div class="form-group">
                <div class="form-group">
                    ...
                    <div class="dropdown-servicos">
                        <button type="button" class="form-input" id="dropdownServicosBtn" onclick="toggleDropdownServicos()">Selecione os serviços</button>
                        <div class="dropdown-servicos-list" id="dropdownServicosList" style="display:none; position:absolute; background:#fff; border:1px solid #e2e8f0; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,0.08); z-index:1000; max-height:220px; overflow-y:auto; width:100%;">
                            <?php foreach ($servicos as $s): ?>
                                <label style="display:flex; align-items:center; gap:8px; padding:6px 12px; cursor:pointer;">
                                    <input type="checkbox" name="servicos_ids[]" value="<?php echo (int)$s['id']; ?>" data-preco="<?php echo (float)$s['preco']; ?>" onchange="atualizarTotalServicosCheckbox()">
                                    <?php echo htmlspecialchars($s['nome']); ?> - R$ <?php echo number_format($s['preco'], 2, ',', '.'); ?>
                                </label>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <small id="totalServicosInfo" style="display:block; color:#0369a1; font-size:0.75rem; margin-top:4px;">
                        Total estimado dos serviços selecionados: <strong id="totalServicosSpan">R$ 0,00</strong>
                    </small>
                    <style>
                        .dropdown-servicos {
                            position: relative;
                        ...
                        .dropdown-servicos-list {
                            min-width: 220px;
                        }

                        #dropdownServicosBtn:after {
                            content: '▼';
                            float: right;
                            margin-left: 8px;
                            font-size: 0.8em;
                    ...
                    <script>
                        function toggleDropdownServicos() {
                            var list = document.getElementById('dropdownServicosList');
                        ...
                        document.addEventListener('click', function(e) {
                            var btn = document.getElementById('dropdownServicosBtn');
                            var list = document.getElementById('dropdownServicosList');
                        ...
                        function atualizarTotalServicosCheckbox() {
                            var total = 0;
                            var checkboxes = document.querySelectorAll('input[name="servicos_ids[]"]:checked');
                            checkboxes.forEach(function(cb) {
                                var preco = parseFloat(cb.getAttribute('data-preco') || '0');
                                total += preco;
                            });
                            // Pega quantidade de recorrências
                            var recorrenciaQtd = 1;
                            var recorrenciaAtiva = document.querySelector('input[name="recorrencia_ativa"]:checked');
                            if (recorrenciaAtiva && recorrenciaAtiva.value === '1') {
                                var inputQtd = document.getElementById('recorrencia_qtd');
                                if (inputQtd && parseInt(inputQtd.value) > 1) {
                                    recorrenciaQtd = parseInt(inputQtd.value);
                                }
                            }
                            var totalFinal = total * recorrenciaQtd;
                            var span = document.getElementById('totalServicosSpan');
                            if (span) {
                        ...
                        // Atualiza o valor total ao mudar a quantidade de recorrências ou ativar/desativar recorrência
                        document.addEventListener('DOMContentLoaded', function() {
                            var inputQtd = document.getElementById('recorrencia_qtd');
                            if (inputQtd) {
                                inputQtd.addEventListener('input', function() {
                                    atualizarTotalServicosCheckbox();
                                });
                            }
                            var radiosRec = document.querySelectorAll('input[name="recorrencia_ativa"]');
                            radiosRec.forEach(function(radio) {
                ...
                <div class="form-group">
                    ...
                    <div style="display:flex; flex-direction:column; gap:6px; margin-bottom:8px;">
                        <label style="font-size:0.75rem; display:flex; align-items:center; gap:6px;">
                            <input type="radio" name="recorrencia_ativa" value="0" checked>
                            Não repetir (agendamento único)
                        </label>
                        <label style="font-size:0.75rem; display:flex; align-items:center; gap:6px;">
                            <input type="radio" name="recorrencia_ativa" value="1">
                            Repetir a cada X dias
                        </label>
                    </div>
                    <div id="recorrenciaConfig" style="display:none; margin-top:4px; display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                        <div>
                            <label class="form-label" style="font-size:0.7rem;">Intervalo (em dias)</label>
                            <input
                                type="number"
                        ...
                        <div>
                            <label class="form-label" style="font-size:0.7rem;">Quantidade de sessões</label>
                            <input
                                type="number"
                    ...
                    <small style="font-size:0.7rem; color:#64748b; display:block; margin-top:6px;">
                        Exemplo: intervalo de <strong>15 dias</strong> e <strong>4 sessões</strong> → cria 4 agendamentos,
                        sempre com 15 dias de diferença a partir da data escolhida.
                    </small>
                </div>
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        // --- Recorrência: mostrar/esconder configuração ---
                        const radiosRec = document.querySelectorAll('input[name="recorrencia_ativa"]');
                        const blocoRec = document.getElementById('recorrenciaConfig');

                        function atualizarVisibilidadeRecorrencia() {
                            if (!blocoRec) return;
                            const selecionado = document.querySelector('input[name="recorrencia_ativa"]:checked');
                            blocoRec.style.display = (selecionado && selecionado.value === '1') ? 'grid' : 'none';
                        }

                        if (radiosRec.length && blocoRec) {
                            radiosRec.forEach(r => r.addEventListener('change', atualizarVisibilidadeRecorrencia));
                            atualizarVisibilidadeRecorrencia();
                        }

                        // --- Mostrar dia da semana abaixo da data ---
                        const inputData = document.querySelector('input[name="data_agendamento"]');
                        const infoDia = document.getElementById('infoDiaSemana');
                        const nomesDias = [
                        ...
                        function atualizarInfoData() {
                            if (!inputData || !infoDia || !inputData.value) return;
                            const d = new Date(inputData.value + 'T00:00:00');
                            if (isNaN(d.getTime())) return;
                            const idx = d.getDay();
                            const dataBR = inputData.value.split('-').reverse().join('/');
                            infoDia.innerHTML = '<i class="bi bi-calendar-event"></i> ' +
                                nomesDias[idx] + ' – ' + dataBR;
                        }

                        if (inputData) {
                            inputData.addEventListener('change', atualizarInfoData);
                            atualizarInfoData(); // já preenche ao abrir
                        }
                    });
...
function renderCard($ag, $clientes)
{
    ...
    foreach ($clientes as $c) if ($c['nome'] === $ag['cliente_nome']) {
        $tel = $c['telefone'];
        break;
    }
...
<script>
    ...
    function openModal() {
        ...
        // dispara o "change" da data só pra atualizar o texto do dia da semana
        const inputData = document.querySelector('input[name="data_agendamento"]');
        if (inputData) {
            const ev = new Event('change');
            inputData.dispatchEvent(ev);
        }
    }
    ...
    function sincronizarCamposFormulario() {
        ...
        // Sincronizar telefone (mobile para desktop)
        var inputTelMobile = document.getElementById('inputTelefoneMobile');
        var inputTel = document.getElementById('inputTelefone');
        if (inputTelMobile && inputTelMobile.value) {
            inputTel.value = inputTelMobile.value;
        }
        // Garante que o campo telefone (desktop) sempre tenha valor para envio
        if (inputTel && !inputTel.value && inputTelMobile && inputTelMobile.value) {
            inputTel.value = inputTelMobile.value;
        }
